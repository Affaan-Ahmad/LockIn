...
| 23 | **Every statically prerendered page shipped with all JavaScript blocked.** The CSP is nonce-based, and a nonce only exists if there is a request to mint it for — a prerendered page is built once with no request, so its script tags carried no nonce while middleware still sent a policy demanding one. `strict-dynamic` made it absolute: browsers honouring it ignore `'self'`, so every script was refused. **Measured on the deployed site 2026-09-09:** `/legal/privacy` served 38 script tags with zero nonces and logged 37 CSP violations; `/legal/terms` 30; `/offline` 21. None of them ran any JavaScript. `/welcome`, which is dynamic, served 46 script tags with 48 nonces and zero violations. It went unnoticed because static text looks correct without JavaScript — it surfaced only when `/offline` was given a client component that had to read local storage. **FIXED IN CODE 2026-09-09:** the legal segment and `/offline` are `force-dynamic`, so they are rendered per request and receive a nonce like every other page. The policy is unchanged and was not weakened. **NOT VERIFIED in production** until the deploy lands. | HIGH | The legal pages are the ones Google's OAuth reviewers read, which is what makes this worse than it looks. Row 4 called `style-src 'unsafe-inline'` the one documented relaxation; this was an undocumented and far larger hole in the same policy, in the opposite direction — it did not permit too much, it forbade everything. |
...
`pg_catalog` and `cron.job` directly, closing a question four separate gaps had been carrying.

| Migration | Evidence | Verdict |
| --- | --- | --- |
| `0007` ignored assignments | table present | Applied |
| `0009` drop grades | both grade columns absent | Applied |
| `0010` sync retention | `pg_cron` installed; `cron.job` lists `lockin-prune-sync-history`, `20 3 * * *`, `active = true` | Applied and scheduled |
| `0013` fail-run fencing | `app_fail_sync_run` source contains `if v_updated = 0 then` | Applied |
...
running. It conflated two unrelated jobs: `0010`'s pruning runs inside Postgres on pg_cron and needs
no secret, while the sweep is a recovery backstop for stranded sync runs and is the only one
`CRON_SECRET` gates. Retention has been running the whole time; the privacy policy was the thing
that was wrong, having claimed pruning was "not yet running on a schedule". It now states the
ninety-day window as fact. See gap 10a.
