...
| 21 | **A complete consent was read as an incomplete one, in production.** Observed 2026-09-06 on `lockinapp.tech`: a student accepted every permission on the Google consent screen and was redirected to `/welcome?connection=insufficient_scopes`, with the connection stored `NEEDS_RECONNECT` / `INSUFFICIENT_SCOPES`. Reconnecting could not help, because a second consent produces the same token and the same verdict. The consent was fine; the comparison was not. Google reports the *permission* it granted rather than the scope string that was asked for, and it collapses `classroom.student-submissions.me.readonly` into `classroom.coursework.me.readonly` — the two are one line on the consent screen, carry the identical description, and cannot be accepted or declined separately — so `tokeninfo` names only the coursework scope back. `missingClassroomScopes` matched Google's answer against the requested list by string identity and called a complete grant one permission short. **FIXED IN CODE 2026-09-06:** the rule now satisfies a required scope with that scope *or* an equivalent conferring at least the same access, one-directionally, so a grant carrying only `student-submissions` still fails the coursework requirement. The requested scope set is unchanged, and the stored `granted_scopes` remains exactly what Google reported. Regression coverage at the domain rule, the callback's connection service and the token service's sync gate. **The fix is NOT VERIFIED against a live Google consent flow** — it corrects a rule that live behaviour disproved, and only a live flow can close it. | ~~HIGH~~ FIXED IN CODE | The first live-flow evidence bearing on row 16, whose closure was explicitly unit-tested only. Row 16's write path was right; its comparison was not. |
...
`classroom.coursework.me.readonly` are one permission to Google — one line on the consent screen,
the identical description, no way to accept one and decline the other — and a token granted both is
described by `tokeninfo` as carrying only the coursework scope. All four are still requested, but
the rule that decides whether a stored grant is usable matches on the access a scope confers rather
than on the string that was asked for. See row 21.
